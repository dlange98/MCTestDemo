...
let WizardTestCommand = class WizardTestCommand extends commandline_1.AppCommand {
    ...
    run(client, portalBaseUrl) {
        return __awaiter(this, void 0, void 0, function* () {
            const getDeviceSets = client.test.listDeviceSetsOfOwner(this.app.ownerName, this.app.appName);
            const getAppOS = client.apps.get(this.app.ownerName, this.app.appName);
            ...
            const devices = yield this.promptDevices(yield getDeviceSets, client);
    ...
    getDevices(client) {
        return __awaiter(this, void 0, void 0, function* () {
            const configs = yield client.test.getDeviceConfigurations(this.app.ownerName, this.app.appName);
    ...
    promptDevices(deviceSets, client) {
        return __awaiter(this, void 0, void 0, function* () {
            let choices;
            const noDeviceSets = deviceSets.length === 0;
            if (noDeviceSets) {
                const devices = yield interaction_1.out.progress("Getting list of devices...", this.getDevices(client));
            ...
            let deviceId;
            if (noDeviceSets) {
                const deviceSelection = yield client.test.createDeviceSelection(this.app.ownerName, this.app.appName, [answers.deviceSlug]);
                deviceId = deviceSelection.shortId;
            }
            else {
                if (answers.deviceSlug === "manual") {
                    return yield this.promptDevices([], client);
                }
                else {
    ...
    printCalabashHelp() {
        interaction_1.out.text(os.EOL + `Interactive mode is not supported. Usage: appcenter test run calabash ${this.interactiveArgs.join(" ")}` +
            os.EOL + os.EOL + "Additional parameters: " +
            os.EOL + `--project-dir: ${help_messages_1.Messages.TestCloud.Arguments.CalabashProjectDir}` +
            os.EOL + `--sign-info: ${help_messages_1.Messages.TestCloud.Arguments.CalabashSignInfo}` +
            os.EOL + `--config-path: ${help_messages_1.Messages.TestCloud.Arguments.CalabashConfigPath}` +
            os.EOL + `--profile: ${help_messages_1.Messages.TestCloud.Arguments.CalabashProfile}` +
            os.EOL + `--skip-config-check: ${help_messages_1.Messages.TestCloud.Arguments.CalabashSkipConfigCheck}`);
    }
    printManifestHelp() {
        interaction_1.out.text(os.EOL + `Interactive mode is not supported. Usage: appcenter test run manifest ${this.interactiveArgs.join(" ")}` +
            os.EOL + os.EOL + "Additional parameters: " +
            os.EOL + `--manifest-path: Path to manifest file` +
            os.EOL + `--merged-file-name: ${help_messages_1.Messages.TestCloud.Arguments.MergedFileName}`);
    }
};
